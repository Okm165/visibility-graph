...
def polygon_cross(v1: Vertex, poly_edges) -> bool:
    """returns True if Vertex v1 is internal to the polygon"""
    v2 = Vertex(INF, v1.y)
    intersect_count = 0
...
def edge_into_polygon(v1: Vertex, v2: Vertex, graph: Graph) -> bool:
    """returns true if the edge from v1 to v2 is interior to its polygon"""
...
def distance(v1: Vertex, v2: Vertex) -> float:
    """returns the distance between two Vertexs"""
    return sqrt((v2.x - v1.x) ** 2 + (v2.y - v1.y) ** 2)


def intersect_vertex(v1: Vertex, v2: Vertex, edge: Edge):
    """returns intersect Vertex where the edge from v1, v2 intersects edge"""
    if v1 in edge:
        return v1
    if v2 in edge:
        return v2
    ...
    if eslope == pslope:
        return None
    intersect_x = (eslope * edge.v1.x - pslope * v1.x + v1.y - edge.v1.y) / (
        eslope - pslope
    )
    intersect_y = eslope * (intersect_x - edge.v1.x) + edge.v1.y
    return Vertex(intersect_x, intersect_y)

    # this variant doesn't work returns floating point calculation errors
    # tdiv = (v1.x-v2.x)*(edge.v1.y-edge.v2.y) - \
    #     (v1.y-v2.y)*(edge.v1.x-edge.v2.x)
    # if tdiv == 0:
    #     return None

    # t = ((v1.x-edge.v1.x)*(edge.v1.y-edge.v2.y) -
    #      (v1.y-edge.v1.y)*(edge.v1.x-edge.v2.x)) / tdiv
    # if not -INTER_ZT <= t or not t <= 1+INTER_ZT:
    #     return None

    # u = ((v1.x-edge.v1.x)*(v1.y-v2.y) -
    #      (v1.y-edge.v1.y)*(v1.x-v2.x)) / tdiv
    # if not -INTER_ZT <= u or not u <= 1+INTER_ZT:
    #     return None

    # return Vertex(v1.x+t*(v2.x-v1.x), v1.y+t*(v2.y-v1.y))


def vertex_distance(v1: Vertex, v2: Vertex, edge: Edge) -> float:
    """returns the distance from v1 to intersect Vertex with edge, 0 if no intersection"""
...
def angle(center: Vertex, vert: Vertex):
    """returns the angle (radian) of Vertex from center of the coordinate system"""
    dx = vert.x - center.x
    dy = vert.y - center.y
    if dx == 0:
        if dy < 0:
...
def angle2(va: Vertex, vb: Vertex, vc: Vertex) -> float:
    """returns angle B (radian) between va-vb and va-vc"""
    a = (vc.x - vb.x) ** 2 + (vc.y - vb.y) ** 2
    b = (vc.x - va.x) ** 2 + (vc.y - va.y) ** 2
    c = (vb.x - va.x) ** 2 + (vb.y - va.y) ** 2
    cos_value = (a + c - b) / (2 * sqrt(a) * sqrt(c))
    return acos(int(cos_value * T1) / T2)


def ccw(a: Vertex, b: Vertex, c: Vertex):
    """returns 1 if counter clockwise, -1 if clock wise, 0 if collinear"""
    det = int(((b.x - a.x) * (c.y - a.y) - (b.y - a.y) * (c.x - a.x)) * T1) / T2
    if det > 0:
        return 1
    if det < 0:
        return -1
...
def edge_intersect(v1: Vertex, q1: Vertex, edge: Edge) -> bool:
    """returns True if edge from A, B interects edge"""
    v2 = edge.v1
    q2 = edge.v2
    o1 = ccw(v1, q1, v2)
    o2 = ccw(v1, q1, q2)
...
def cmp_edges(v1: Vertex, v2: Vertex, edge1: Edge, edge2: Edge) -> float:
    """returns True if edge1 is smaller than edge2"""
    if edge1 == edge2:
        return False
    if not edge_intersect(v1, v2, edge2):
        return True
    dist1 = vertex_distance(v1, v2, edge1)
    dist2 = vertex_distance(v1, v2, edge2)
    if dist1 > dist2:
        return False
    if dist1 < dist2:
        return True
    ...
    if edge1.v1 in edge2:
        ...
        common = edge1.v2
    if angle2(v1, v2, edge1.get_adjacent(common)) < angle2(
        v1, v2, edge2.get_adjacent(common)
    ):
        return True
    return False
